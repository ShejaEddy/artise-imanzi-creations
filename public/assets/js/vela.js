...
Shopify.updateOptionsInSelector = function(selectorIndex) {
    ...
    jQuery(
        ...
        if (
            jQuery.inArray($(this).attr("data-value"), availableOptions) !== -1
        ) {
            $(this)
                .removeClass("soldout")
                .show()
...
vela.productPage = function(options) {
    ...
    if (variant) {
        ...
        if (window.swatch_enable) {
            ...
            for (var i = 0, length = variant.options.length; i < length; i++) {
                form.find('.swatch[data-option-index="' + i + '"] :radio').each(
                    function() {
                        if (_.isEqual(variant.options[i], $(this).val())) {
                            $(this).prop("checked", true);
                            var headerValue = form.find(
        ...
        if (variant.available) {
            ...
            $(".productAvailability").html(
                "<label>" + "Availability" + ":</label> " + "Out Of Stock"
            );
        }
        if (window.currencies) {
            Currency.convertAll(
                window.currency,
                $(".jsvela-currency__item.active").data("value"),
                "span.money",
                "money_format"
            );
        }
    ...
    if (variant && variant.featured_media) {
        ...
        var mediaID,
            slideIndex = 0;
        $("#productThumbs .product-single__thumbnail").removeClass(
            "active-thumb"
        );
        $("#productThumbs .product-single__thumbnail").each(function() {
            ...
            if (mediaID === currentMediaID) {
                var currentSlide = $self.closest(
                    ".product-single__thumbnails-item"
                );
                slideIndex = currentSlide.data("slick-index")
                    ? currentSlide.data("slick-index")
                    : 0;
                $self.addClass("active-thumb");
                $("#ProductPhotoImg")
                    .attr("src", $self.data("image"))
                    .attr("data-zoom-image", $self.data("zoom-image"));
                $self.trigger("click");
                return false;
            }
        });
        if (slideIndex != "undefined") {
            $("#productThumbs .product-single__thumbnails").slick(
                "slickGoTo",
                slideIndex
            );
        }
...
vela.startTheme = function() {
    ...
    $(".headerCartModal .overlayCart, .headerCartModal .closeCartModal").on(
        "click",
        function() {
...
vela.swatchProduct = function() {
    $(".velaSwatchProduct > li").each(function() {
        var eImage = $(this)
            .parents(".velaProBlock")
            .find(".proSwatch .product-card__image > img");
        ...
        $(this).mouseover(function() {
            var newImage = $(this)
                ...
                .attr("href");
            if (newImage != "#") {
                eImage.attr({ src: newImage, srcset: "" });
            }
        });
        $(this).mouseout(function() {
            var eImgsrcset = eImage.data("srcset");
            eImage.attr({ srcset: eImgsrcset });
    ...
    if ($("#productThumbs").length > 0) {
        $("#productThumbs .owl-carousel").each(function() {
            ...
            var config = {
                ...
                vertical: vertical,
                responsive: [
                    { breakpoint: 768, settings: { slidesToShow: mItem } },
                    {
                        breakpoint: 992,
                        settings: { slidesToShow: item, slidesToScroll: item }
                    },
                    { breakpoint: 1200, settings: { slidesToShow: mItem } },
                    { breakpoint: 1400, settings: { slidesToShow: mItem } }
                ]
            };
                            ...
                            function(data) {
                                ...
                                if (data.results_count == 0) {
                                    ...
                                    $.each(data.results, function(index, item) {
                                        ...
                                        link.append(
                                            '<span class="searchProductImage"><img src="' +
                                                item.thumbnail +
                                                '" /></span>'
                                        );
                                        link.append(
...
vela.ajaxFilter = function() {
    ...
    vela.cache.$body.on(
        "click",
        ".filterTagFullwidthOverlay, .filterTagFullwidthClose",
        function() {
            $(".filterTagFullwidthOverlay").each(function() {
                $(this).remove();
            });
    ...
    ajaxFilterPaging = function() {
        vela.cache.$body.on("click", "#collPagination .pagination a", function(
            event
        ) {
            event.preventDefault();
            var linkPage = $(this)
            ...
            if (linkPage) {
                ...
                if (Shopify.queryParams.page) {
                    ...
                    ajaxFilterGetContent(newurl);
                    $("body,html").animate({ scrollTop: 300 }, 600);
                }
            }
        });
    };
    ...
    ajaxFilterData = function(data) {
        ...
        if ($("#collPagination").length > 0) {
            ...
            $("#shopify-section-vela-template-collection").append(
                $(data).find("#collPagination")
            );
        }
...
vela.responsiveVideos = function() {
    var $iframeVideo = $(
        '.proDetailInfo iframe[src*="youtube.com/embed"], .proDetailInfo iframe[src*="player.vimeo"]'
    );
...
vela.menuMobile = function() {
    ...
    vela.cache.$body.on("click", ".btnMenuClose, .menuMobileOverlay", function(
        e
    ) {
        e.preventDefault();
...
vela.owlOneCarousel = function() {
    $(".owlCarouselPlay .owl-carousel").each(function() {
        ...
        dots === undefined || dots == null || dots.length <= 0 || dots != true
            ? (config.dots = false)
            : (config.dots = true);
        navText === undefined || navText == null || navText.length <= 0
            ? true
            : (config.navText = navText);
        loop === undefined || loop == null || loop.length <= 0
            ? true
            : (config.loop = loop);
        center === undefined || center == null || loop.center <= 0
            ? true
...
vela.lookbook = function() {
    vela.cache.$body.on("click", ".lookbItemButton", function() {
        ...
        if (!boxLookBook.hasClass("active")) {
            ...
            itemLookBookContent.prepend(
                '<div class="velaBoxLookbookClose"></div>'
            );
    ...
    vela.cache.$body.on(
        "click",
        ".velaBoxLookbookOverlay, .velaBoxLookbookClose",
        function() {
...
vela.quickview = function() {
    ...
    selectCallbackQuickView = function(variant, selector) {
        ...
        if (variant) {
            ...
            productItem
                .find(".quickViewSKU")
                .html("<label>SKU:</label>" + variantSku);
            if (variant.available) {
                ...
                productItem
                    .find(".proBoxInfo .quickviewAvailability")
                    .append("<label>Availability</label>In stock");
            } else {
                ...
                productItem
                    .find(".proBoxInfo .quickviewAvailability")
                    .append("<label>Availability</label>Out Of Stock");
            }
    ...
    velaUpdateOptionsInSelector = function(t) {
        switch (t) {
            ...
            case 2:
                ...
                n +=
                    " / " +
                    $(".jsQuickview .single-option-selector:eq(1)").val();
        ...
        $(
            '.jsQuickview .swatch[data-option-index="' +
                t +
                '"] .swatch-element'
        ).each(function() {
            var variantValue = $(this)
                .find(".js-qv-variant-value")
                .text();
    ...
    velaLinkOptionSelectors = function(t) {
        for (var n = 0; n < t.variants.length; n++) {
            var r = t.variants[n];
            if (r.available) {
                Shopify.optionsMapQuickview["root"] =
                    Shopify.optionsMapQuickview["root"] || [];
                Shopify.optionsMapQuickview["root"].push(r.option1);
                Shopify.optionsMapQuickview["root"] = Shopify.uniq(
                    Shopify.optionsMapQuickview["root"]
                );
                if (t.options.length > 1) {
                    var i = r.option1;
                    Shopify.optionsMapQuickview[i] =
                        Shopify.optionsMapQuickview[i] || [];
                    Shopify.optionsMapQuickview[i].push(r.option2);
                    Shopify.optionsMapQuickview[i] = Shopify.uniq(
                        Shopify.optionsMapQuickview[i]
                    );
                }
        ...
        if (t.options.length > 1) velaUpdateOptionsInSelector(1);
        if (t.options.length === 3) velaUpdateOptionsInSelector(2);
    ...
    loadQuickViewSlider = function(n, r) {
        ...
        if (n.media.length > 0) {
            ...
            for (i in n.media) {
                ...
                var f =
                    '<div class="thumbItem"><a href="javascript:void(0)" class="' +
                    activeClass +
                    '"  data-index="' +
                    i +
                    '" data-imageid="' +
                    n.media[i].id +
                    '" data-image="' +
                    n.media[i].preview_image.src +
                    '" data-zoom-image="' +
                    n.media[i].preview_image.src +
                    '" ><img src="' +
                    n.media[i].preview_image.src +
                    '" alt="' +
                    n.media[i].preview_image.alt +
                    '" /></a></div>';
                o.append(f);
            }
    ...
    quickViewVariants = function(t, quickview) {
        if (t.variants.length > 1) {
            for (var r = 0; r < t.variants.length; r++) {
                ...
                quickview
                    .find("form.formQuickview .proVariantsQuickview > select")
                    .append(s);
            }
            ...
            if (t.options.length == 1) {
                $("form.formQuickview .selector-wrapper:eq(0)").prepend(
                    "<label>" + t.options[0].name + "</label>"
                );
            }
            quickview
                .find("form.formQuickview .selector-wrapper label")
                .each(function(n, r) {
                    $(this).html(t.options[n].name);
                });
            if (window.swatch_enable) {
                var o = window.file_url.substring(
                    0,
                    window.file_url.lastIndexOf("?")
                );
                var u = window.asset_url.substring(
                    0,
                ...
                for (var r = 0; r < t.options.length; r++) {
                    ...
                    a +=
                        '<div class="header">' +
                        t.options[r].name +
                        ": <span class='js-swatch-display text'>&nbsp;</span></div>";
                    var f = false;
                    ...
                    for (var c = 0; c < t.variants.length; c++) {
                        ...
                        if (l.indexOf(h) < 0) {
                            ...
                            a +=
                                ...
                                (f ? "color " : "") +
                                p +
                                (i.available ? " available " : " soldout ") +
                                '">';
                            if (f) {
                                a += '<div class="tooltip">' + h + "</div>";
                            }
                            a +=
                                '<span class="js-qv-variant-value hidden">' +
                                h +
                                "</span>";
                            a +=
                                '<input id="' +
                            ...
                            if (f) {
                                a +=
                                    ...
                                    bgImage +
                                    '.png)"><img class="crossed-out" src="' +
                                    u +
                                    'soldout.png" /><i></i></label>';
                            } else {
                                a +=
                                    '<label class="' +
                                    p +
    ...
    $(document).on("click", ".btnProductQuickview", function(e) {
        ...
        Shopify.getProduct(producthandle, function(product) {
            ...
            var productdes = product.description.replace(/(<([^>]+)>)/gi, "");
            ...
            if (product.description.indexOf("[SHORTDESCRIPTION]") != -1) {
                shortProductDesc = product.description.split(
                    "[SHORTDESCRIPTION]"
                )[0];
            ...
            quickview
                .find(".pricePrimary")
                .html(Shopify.formatMoney(product.price, window.money));
            quickview.find(".proBoxInfo").attr("id", "product-" + product.id);
            quickview
                .find(".formQuickview")
                .attr("id", "product-actions-" + product.id);
            quickview
                .find(".formQuickview select")
                .attr("id", "productSelectQuickview");
            quickview
            ...
            quickview
                .find(".proBoxInfo .quickViewVendor")
                .append("<label>Vendor:</label> " + product.vendor);
            ...
            quickview
                .find(".proBoxInfo .quickViewSKU")
                .append("<label>SKU:</label> " + variantSku);
...
vela.newsletter = function() {
    ...
    alertNewsletter = function(message, type) {
        var alert =
            '<div class="js-alert-newsletter alert-dismissible  alert alert-' +
            type +
            '">' +
            message +
            '<button type="button" data-dismiss="alert" aria-hidden="true" aria-label="Close" class="btnClose">x</button></div>';
        return alert;
    };
};
    ...
    if (vela.cache.$velaProductImage.length > 0) {
        if ($(window).width() >= 992) {
            var zoomYN = vela.cache.$velaProductImage.data("zoom-enable"),
                ...
                zoomWindowWidth = vela.cache.$velaProductImage.data(
                    "zoom-width"
                ),
                zoomWindowHeight = vela.cache.$velaProductImage.data(
                    "zoom-height"
                ),
                zoomLensSize = vela.cache.$velaProductImage.data("zoom-lens"),
                zoomLensShape = vela.cache.$velaProductImage.data("lens-shape"),
                zoomLensBorderColor = vela.cache.$velaProductImage.data(
                    "lens-border"
                );
...
velatheme.Slideshow = (function() {
    ...
    function beforeChange(event, slick, currentSlide, nextSlide) {
        ...
        var $nextSlide = $slider.find(
            '.velassSlide[data-slick-index="' + nextSlide + '"]'
        );
...
velatheme.SlideshowVideo = (function() {
    ...
    function getVideoOptions(evt) {
        var videosid = $(".slideShowVela .slick-active .velassVideo").attr(
            "id"
        );
    ...
    function sizeBackgroundVideo($player) {
        ...
        if (slideWidth / videoOptions.ratio < playerHeight) {
            ...
            $player
                .width(playerWidth)
                .height(playerHeight)
                .css({ left: (slideWidth - playerWidth) / 2, top: 0 });
        } else {
            playerHeight = Math.ceil(slideWidth / videoOptions.ratio);
            $player
                .width(slideWidth)
                .height(playerHeight)
                .css({ left: 0, top: (playerHeight - playerHeight) / 2 });
        }
...
velatheme.SlideshowSection.prototype = _.assignIn(
    {},
    velatheme.SlideshowSection.prototype,
    {
        onUnload: function() {
        ...
        onBlockSelect: function(evt) {
            ...
            var $slide = $(
                ".velassSlide" + evt.detail.blockId + ":not(.slick-cloned)"
            );
