          ...
          .each(function () {
            if (_.isEqual(variant.options[i], $(this).val())) {
              ...
              var headerValue = form.find(
                '.swatch[data-option-index="' + i + '"] .js-swatch-display'
              );
          ...
          currentAjaxRequest = $.getJSON(searchURL + "&view=json", function (
            ...
            if (data.results_count == 0) {
              ...
              $.each(data.results, function (index, item) {
                ...
                link.append(
                  '<span class="searchProductTitle">' + item.title + "</span>"
                );
                link.wrap("<li></li>");
...
vela.quickview = function () {
  ...
  selectCallbackQuickView = function (variant, selector) {
    ...
    if (variant) {
      ...
      if (window.swatch_enable) {
        ...
        for (var i = 0, length = variant.options.length; i < length; i++) {
          form
            .find('.swatch[data-option-index="' + i + '"] .js-qv-variant-value')
            .each(function () {
  ...
  loadQuickViewSlider = function (n, r) {
    ...
    if (n.media.length > 0) {
      ...
      o.owlCarousel({ items: 4, nav: true, mouseDrag: false, dots: false }).css(
        "visibility",
        "visible"
      );
    } else {
  ...
  $(document).on("click", ".btnProductQuickview", function (e) {
    ...
    Shopify.getProduct(producthandle, function (product) {
      ...
      if (product.description.indexOf("[SHORTDESCRIPTION]") != -1) {
        ...
        shortProductDesc =
          productdes.split(" ").splice(0, 30).join(" ") + "...";
      ...
      quickview
        ...
        .html("<a href='" + product.url + "'>" + product.title + "</a>");
      quickview
      ...
      if (product.compare_at_price > product.price) {
        quickview
          .find(".priceCompare")
          .html(Shopify.formatMoney(product.compare_at_price_max, window.money))
          .show();
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data.result === "success") {
            $form.prepend(
              alertNewsletter(window.newsletter_success, "success")
            );
            $(".js-input-newsletter").val("");
          } else {
            $form.prepend(
              alertNewsletter(data.msg.replace("0 - ", ""), "error")
            );
          }
        },
        error: function (err) {
