    ...
    success: function (line_item) {
      ...
      if (window.ajaxcart_type == "modal") {
        if (line_item.image != null) {
          $(".headerCartModal")
            .find(".cartProductImage img")
            .attr("src", line_item.image);
        } else {
          $(".headerCartModal")
            .find(".cartProductImage img")
            .attr("src", "//placehold.it/100x100");
        }
...
var ajaxCart = (function (module, $) {
  ...
  itemErrorCallback = function (form, XMLHttpRequest, textStatus) {
    ...
    if (!!data.message) {
      if (data.status == 422) {
        $(form).after(
          '<div class="alert alert-danger qtyError">' +
            data.description +
            "</div>"
        );
      }
    ...
    if (window.ajaxcart_type == "modal") {
      if (cart.item_count === 0) {
        $cartContainer.append(
          '<div class="headerCartEmpty">' + window.cart_empty + "</div>"
        );
        cartCallback(cart);
        return;
      }
      ...
      if (cart.item_count === 0) {
        $cartContainer.append(
          '<div class="drawerCartEmpty">' + window.cart_empty + "</div>"
        );
        cartCallback(cart);
        return;
      }
  ...
  adjustCart = function () {
    ...
    function updateQuantity(line, qty) {
      isUpdating = true;
      var $row = $('.ajaxCartRow[data-line="' + line + '"]').addClass(
        "is-loading"
      );
      if (qty === 0) {
